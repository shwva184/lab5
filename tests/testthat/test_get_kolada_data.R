...
test_that("getMetadata() is working", {
  ...
  expect_equal(df[6, "Municipality_ID"], "2506")
  expect_equal(df[6, "Title"], "Arjeplog")
  df = getMetadata("kpi")
  expect_true(is.data.frame(df))
  expect_equal(df[3, "KPI_ID"], "N00005")
  expect_equal(df[3, "Title"], "Utjämningssystemet enl resultaträkning, kr/inv")
  df = getMetadata("ou")
  expect_true(is.data.frame(df))
  expect_equal(df[5, "Ou_ID"], "V15E011401301")
  expect_equal(df[5, "Title"], "Sverigefinska sk, Uppl Väsby")
